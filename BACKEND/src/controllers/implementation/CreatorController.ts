    ...
    try {
      ...
      try {
        ...
        if (
          ...
          return res.status(StatusCodes.BAD_REQUEST).json({ message:CreatorMessages.INVALID_GEOLOCATION_FORMAT });
        }
      } catch (err) {
        return res.status(StatusCodes.BAD_REQUEST).json({ message: CreatorMessages.INVALID_GEOLOCATION_JSON });
      }
